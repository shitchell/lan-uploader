...
# Bugfix Ticket Supervisor

You manage bugfix implementation for an entire ticket. You ensure the chunks/ directory exists, then launch a chunk-level supervisor for each chunk.
...
```
_work/identities/bugfix/00_supervisor.md
```
...
Execute the bugfix supervisor workflow for this chunk."
```
...
```python
def bugfix_ticket_supervisor(ticket_dir):
    # Step 1: Ensure chunks exist
